          ...
          <form id="data_form" action="" method="post">
            <div class="box-body">
              <div class="row">
                ...
                $this->db->select_max('kode_opname');
                $get_max_po = $this->db->get_where('transaksi_opname',array('status_opname'=>'Nominal'));
                $max_po = $get_max_po->row();
                //echo $this->db->last_query();
                $this->db->where('kode_opname', $max_po->kode_opname);
                $get_po = $this->db->get('transaksi_opname');
                $po = $get_po->row();
                //echo $this->db->last_query();
                $tahun = substr(@$po->kode_opname, 3,4);
                if(date('Y')==$tahun){
                  $nomor = substr(@$po->kode_opname, 8);
                  $nomor = $nomor + 1;
                  $string = strlen($nomor);
                  if($string == 1){
                    $kode_trans = 'OP_'.date('Y').'_00000'.$nomor;
                  } else if($string == 2){
                    $kode_trans = 'OP_'.date('Y').'_0000'.$nomor;
                  } else if($string == 3){
                    $kode_trans = 'OP_'.date('Y').'_000'.$nomor;
                  } else if($string == 4){
                    $kode_trans = 'OP_'.date('Y').'_00'.$nomor;
                  } else if($string == 5){
                    $kode_trans = 'OP_'.date('Y').'_0'.$nomor;
                  } else if($string == 6){
                    $kode_trans = 'OP_'.date('Y').'_'.$nomor;
                  }
                } else {
                  $kode_trans = 'OP_'.date('Y').'_000001';
                }

                ?>
                <!--<?php @$hasil_kode_opname->kode_opname."_".date("dmyHis")."_".$no_belakang ?>-->
                <div class="col-md-6">
                  <div class="box-body">
                    <div class="btn btn-app blue">
                      ...
                      <input readonly="true" type="hidden" value="<?php echo $kode_trans ?>" class="form-control" placeholder="Kode Transaksi" name="kode_opname" id="kode_opname" />
                    </div>
                  </div>
                </div>
