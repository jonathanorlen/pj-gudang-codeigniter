<div class="row">      
        ...
        <div class="box-body">                   
          ...
          <form id="data_form" action="" method="post">
            ...
            <div class="box-body">
              ...
              $tgl = date("Y-m-d");
              $no_belakang = 0;
              $this->db->select_max('kode_repack');
              $kode = $this->db->get_where('transaksi_repack',array('tanggal_update'=>$tgl));
              $hasil_kode = $kode->row();
                                        #$pecah_kode = explode("_",$hasil_kode_pembelian->kode_pembelian);
                                        #echo $pecah_kode[0];
                                        #echo $pecah_kode[2];
              $this->db->select('kode_repack');
              $kode_repack = $this->db->get('master_setting');
              $hasil_kode_repack = $kode_repack->row();

              if(count($hasil_kode)==0){
                $no_belakang = 1;
              }
              else{
                $pecah_kode = explode("_",$hasil_kode->kode_repack);
              ...
              ?>
              <input type="text" value="<?php echo @$hasil_kode_repack->kode_repack."_".date("dmyHis")."_".$no_belakang ?>" class="form-control" placeholder="Kode Transaksi" name="kode_repack" id="kode_repack" readonly/>                           
            ...
            <div class="box-body">
              <div class="row">
                <div class="">
                  <div class="col-md-3">
                    <label>Produk </label>
                    <select name="kategori_barang" id="kategori_barang" class="form-control select2" tabindex="-1" aria-hidden="true">
                      <option value="" selected="true">--Pilih Produk--</option>
                      <?php
                      $produk = $this->db->get('master_bahan_baku');
  ...
  $('#kategori_barang').on('change',function(){
    ...
    var url = "<?php echo base_url() . 'repack/get_satuan' ?>";
    $.ajax({
      type: 'POST',
      url: url,
      dataType:'json',
      data: {kategori_barang:kategori_barang,produk:produk},
      ...
      if(unit_awal=='' && rak_awal=='' && unit_akhir=='' && rak_akhir=='' ){
        ...
        $('#kode_bahan').on('change',function(){
          ...
          $.ajax({
            ...
            url: url,
            dataType:'json',
            data: {produk:produk,jenis_bahan:jenis_bahan,kategori_barang:kategori_barang,unit_awal:unit_awal},
            success: function(msg){
...
function add_item(){
  ...
  var kode_repack = $('#kode_repack').val();
  var produk = $('#kategori_barang').val();


  var url = "<?php echo base_url().'repack/simpan_item_repack_temp/'?> ";
  ...
  $.ajax({
    ...
    success: function(hasil)
    {
      ...
      var num = data[0];
      var pesan = data[1];
      $("#tabel_temp_data_mutasi").load("<?php echo base_url().'repack/tabel_item_repack_temp/'; ?>"+kode_repack);
        //$('#kategori_bahan').val('');
        $('#kode_bahan').val('');
        $('#kategori_barang').val();
        $('#jumlah').val('');
...
function actEdit(id) {
  var id = id;
  var kode_repack = $('#kode_repack').val();
  var url = "<?php echo base_url().'repack/get_temp_repack'; ?>";
  $.ajax({
    type: 'POST',
    url: url,
    dataType: 'json',
...
function update_item(){
  ...
  var url = "<?php echo base_url().'repack/ubah_item_repack_temp/'?> ";

  $.ajax({
    type: "POST",
    url: url,
    data: { /*kode_repack:kode_repack,
      kategori_bahan:kategori_bahan,
      unit_awal:unit_awal,
      rak_awal:rak_awal,
      kode_bahan:kode_bahan,
...
function delData() {
  var id = $('#id-delete').val();
  var kode_repack = $('#kode_repack').val();
  var url = "<?php echo base_url().'repack/hapus_temp'; ?>";
  $.ajax({
    type: 'POST',
    url: url,
    data: {id:id},
